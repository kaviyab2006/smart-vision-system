...
# Detection loop
# ----------------------------
print("🎥 Camera is on. Say 'Find [object]' to track.")
...
while True:
    ...
    # Only process if a specific object is requested
    if specific_object and results[0].boxes is not None and len(results[0].boxes) > 0:
        for box in results[0].boxes:
            cls_index = int(box.cls.item())
            name = model.names[cls_index]

            if name != specific_object:
                continue
